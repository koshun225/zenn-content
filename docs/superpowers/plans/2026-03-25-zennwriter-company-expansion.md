...
- Process:
  ...
  3. Create `brief.md` from article-spec.md (convert format, fill missing params with heuristics)
...
- Process:
  ...
  5. Tailor instructions based on `article_type` (tool-intro vs comparison vs tutorial vs deep-dive)
...
- Input: `docs/articles/<slug>/code-spec.md`, code at `/Users/koshun/projects/<slug>/`, `docs/articles/<slug>/code-report.md`
- Process:
...
- Process:
  ...
  7. If revision requested: read tech-review.md, copy-review.md, security-review.md for feedback. Fix all issues in one pass. Update article.
